...
namespace motioncam {
            ...
            const CameraDescription& cameraDesc, const bool preferRaw12, const bool preferRaw16, OutputConfiguration& rawConfiguration)
    {
        ...
        if (rawIt != outputConfigs.end()) {
            ...
            // Pick the configuration likely to be the binned version of the full sensor resolution
            if (!configurations.empty()) {
                const int ultraHighPixels = 6000 * 5000;

                // Hack for now. Choose second resolution if ultra high resolution detected
                if(configurations[0].outputSize.width() * configurations[0].outputSize.height() > ultraHighPixels) {
                    if(configurations.size() > 1)
                        rawConfiguration = configurations[1];
                }
                else {
